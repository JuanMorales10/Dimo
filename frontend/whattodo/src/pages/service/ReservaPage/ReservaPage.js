...
function ReservaPage() {
  ...
  const handleSubmit = async (reserva) => {
    try {
      ...
      const fechaInicio = moment(reserva.fecha).set({
        hour: parseInt(horaParts[0]),
        minute: parseInt(horaParts[1]),
        second: 0
      });
      ...
      if (!fechaInicio.isValid()) {
        throw new Error("Fecha y hora de inicio no válidas");
      }
  
      const fechaFin = moment(fechaInicio).add(durationInMinutes, 'minutes');
  
      // Formatear fechas para el envío
      const startDateTimeLocal = fechaInicio.format();
      const endDateTimeLocal = fechaFin.format();
